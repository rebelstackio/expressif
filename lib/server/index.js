...
class Server {
	...
	constructor(config = {}) {
		...
		if ( !global.LOGGER ) {
			...
			LOGGER.info(`Expressif set you LOGGER as global with the ExpressifBasicLogger component`);
		}
		...
		this.app.use(bodyParser.urlencoded(this._config.bodyparser));
	...
	start() {
		...
		if (socket) {
			...
			this.server = this.app.listen(socket, function () {
				...
				LOGGER.info(`App is running on ${socket}`);
			}.bind(this));
		} else {
			// Port
			this.app.set('port', (this._config.port || 8888));
			this.server = this.app.listen(this.app.get('port'),  () => {
				LOGGER.info(`App is running on port ${this.app.get('port')}`);
			});
		}
	}
