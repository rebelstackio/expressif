...
class Server {
	...
	constructor(config = {}, dependecies = {}) {
		...
		// TODO: Check https://github.com/helmetjs/helmet and more security headers
		...
		// Service static file like .html, .js. css
		// and external service(e.g nginx) should do it instead of the backend server
		if ( this._config.statics && this._config.statics.length > 0 ) {
			this._config.statics.forEach(function (statc) {
		...
		if (Boolean(this._config.strict_routing)) {
		...
		// Let response with template files
		// TODO: implement
		if (this._config.templates && this._config.templates.length > 0) {
