 ...
 * @param {Array} middlewares Array of middlewares. Default to []
...
const ServerFactory = ( options, { express = exp, console = global.console, req = require, fs = fsm, path = pathm, JSONValidator = jv } = { express:exp, console:global.console, req:require, fs:fsm, path: pathm, JSONValidator: jv }) => {
	...
	// Check trust_proxy
	if ( typeof _options.trust_proxy === 'boolean' && _options.trust_proxy ) {
		global.LOGGER.debug(`Expressif::Enabling trust proxy...`);
		_app.enable('trust proxy');
	} else if ( typeof _options.trust_proxy !== 'boolean') {
		_app.set('trust proxy', _options.trust_proxy);
	}

	// Apply middlewares
	_options.middlewares.forEach( mdlw => _app.use(mdlw));
