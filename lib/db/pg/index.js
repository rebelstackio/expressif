...
class PGPool {
	...
	constructor(dbconfig = {}, dependecies = {}) {
		...
		this.pool.on('connect', ( client ) => {
			// this._logger.debug(`New client connected`, client);
		});

		this.pool.on('remove', ( client ) => {
			// this._logger.debug(`CLient removed`, client);
		});
	}
	...
	query(statement, params = [], next) {
		...
		if (/* process.env.PGPROFILE && process.env.PGPROFILE != "false"*/ true) {
	 ...
	 * TODO: Enable profiler for async
	 */
