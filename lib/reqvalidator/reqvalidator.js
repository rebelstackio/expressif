...
const getKey = function getKey (val) {
    let rxenum = REQUEST_EXCEPTION_ENUM;
...
const validate = function validate (rex) {
    let rxdef = REQUEST_EXCEPTION_DEFINITONS;
    return function (request, response, next) {
        // TODO: perform bitwise to turn rx (int) to bitwise array
        let arrRX = toBitwiseArray(rex);
        for (let rx of arrRX) {
            let key = getKey(rx);
...
const printRXValid = function printRXValid (validateMask) {
    ...
    let rxdef = REQUEST_EXCEPTION_DEFINITONS;
    let arrRX = toBitwiseArray(validateMask);
    for (let rx of arrRX) {
        let key = getKey(rx);
        if (rxdef[key]) {
            let cond = rxdef[key];
