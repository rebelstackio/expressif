...
const buildOptionsRoute = function buildOptionsRoute(optionsData) {
	const mws = [];
...
Router.buildRouter = function buildRouter(options) {
	...
	const { auth: optAuth, jsonv: jvq, routes } = options;
	...
	if (options.addOptionsRoute) {
		router.options('/', buildOptionsRoute(optionsResponseBody));
	}
