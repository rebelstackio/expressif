...
const authvalidatormw = require('../../middlewares/authvalidor');
...
const RouterFactory = function _RouterFactory(routes, expOpts = {}, dependecies ){
	...
	return routes.map( routedefinition  => {
		...
		try {
			let mws = [];
			const {
				method, path, rprivs, dprivs, rxvalid, validreq, mwares, rroles, droles, public:openendpoint
			} = routedefinition;

			// 1) Apply auth middlewares if exists and if the endpoint is not public
			if ( !openendpoint && ( rprivs || rroles || droles || dprivs ) ) {
				// switch(true){
				...
				// 	break;
				// default:
				// 	global.LOGGER.warn(`Expressif::Ignoring middleware:`, auth);
				// }
				mws.push(authvalidatormw({ rprivs, rroles, droles }, authv ));
			}
