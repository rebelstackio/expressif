	...
	addRoute ( routedef, rebuildOptsRoute ) {
		...
		function buildOptionsData( rt, jv, auth ) {
			...
			switch ( true ) {
				...
				case Boolean(rt.validreq): od.request_schema = jv.ajv.getSchema(rt.validreq).schema;
				case Boolean(rt.validres): od.response_schema = jv.ajv.getSchema(rt.validres).schema;
