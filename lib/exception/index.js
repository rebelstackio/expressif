...
Exception.prototype.toString = function exceptionToString(){
	return `${this.message} | httpstatus:${this.httpstatus} | code:${this.code}`
}
...
class RangeNotSatisfiable extends Exception {
	constructor(message, code) {
		super(message, code || codes.RANGE_NOT_SATISFIABLE, 416);
	}
}
...
class Redirect extends Exception {
	constructor(message, code, redirectObject, ) {
		super(message, code || codes.REDIRECT, 307);
...
class NotModified extends Exception {
	constructor(message, code) {
		super(message, code || codes.NOT_MODIFIED, 304);
	}
}
