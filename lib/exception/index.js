...
class FacebookError extends StackError {
	constructor(errobj, message, code) {
		super(errobj, message, code || exception.codes.FB_ERROR, 500);
	}
};
...
exception.FacebookError = FacebookError;
...
class ExternalServiceError extends Exception {
	constructor(errobj, servicetype) {
		...
		if (errobj && errobj.response && errobj.response.statusMessage) {
			message = errobj.response.statusMessage;
		} else {
			message = `Unexpected External Error from ${servicetype}`;
		}

		if (errobj && errobj.response && errobj.response.statusCode) {
