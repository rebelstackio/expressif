...
class BadAuth extends Exception {
	constructor(message, code) {
		super(message, code || ERROR.codes.BAD_AUTH, 401);
	}
}
...
class InvalidJSON extends Exception {
	constructor ( message, errorData, code ) {
		super ( message, code || Exception.ERROR.codes.INVALID_JSON, 400 );
		this.info = errorData;
	}
}
...
class Unauthorized extends Exception {
	constructor(message, code) {
		super(message, code || Exception.ERROR.codes.UNAUTHORIZED, 401);
	}
}

class Unavailable extends Exception {
	constructor(message, code, retryafter) {
		super(message, code || Exception.ERROR.codes.UPDATING_CACHE, 503);
