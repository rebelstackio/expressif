...
/**
 * Wrap respose object with an standar format and set option for custom behaviors before responde
 * @param {object} data Respond JSON object
 * @param {string} path Request path from express [OPTIONAL]
 * @param {object} options Additional Options to the wrapped object that change the behavior [OPTIONAL]
 */
const wrapSuccessData = function wrapSuccessData(data={}, path, options) {
	...
	return Object.assign(
		{},
		{ data },
		_path ? { path: _path } : {},  // Set path only if it is valid
		{ options: _options }
	);
};
