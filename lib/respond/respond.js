...
const unavailableRetry = function unavailableRetry(response, request, errObject) {
	...
	const resJSON = buildResponse(request, path, code, responseData, false);
	return response.status(statusCode).send(JSON.stringify(resJSON));
};
...
const forbidden = function forbidden(response, request, errObject) {
	...
	response.set('Content-Type', 'application/json');
	const resJSON = this.buildResponseObj(request, type, code, responseData, false);
	return response.status(statusCode).send(JSON.stringify(resJSON));
};
...
const serverError = function serverError(response, request, errObj) {
	...
	const type = 'error';
	const passthrough = buildPassthroughObj(request.body, request.params);
	const resJSON = this.buildResponseObj(passthrough, type, code, responseData, false);
	response.set('Content-Type', 'application/json');
	return response.status(statusCode).send(JSON.stringify(resJSON));
};
