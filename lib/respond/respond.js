...
const forbidden = function forbidden(response, request, errObj) {
	...
	const resJSON = this.buildResponseObj(responseData, options);
...
const success = function success(
	...
	if (wd) {
		...
		const options = {};
		options.passthrough = buildPassthroughObj(rq);
	...
	if (rq && rq.body) {
		const resp = buildResponse(null, { passthrough: buildPassthroughObj(rq) });
		if (resp) {
...
const unavailableRetry = function unavailableRetry(response, request, errObj) {
	...
	if (errObj) {
		...
		options.tyoe = 'Service_Unavailable_Please_Retry';
	}
